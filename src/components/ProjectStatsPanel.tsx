...
export default function ProjectStatsPanel({ stats, loading }: ProjectStatsPanelProps) {
  ...
  const hourPercent = Math.min(100, Math.round((stats.totalHours / (stats.budgetedHours || 1)) * 100)) || 0;

  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6" id={`project-stats-panel-${stats.projectId}`}>
      
      {/* Selection & Global Hito Panel (Left side of Stats Bento, col-span-5) */}
      <div className="lg:col-span-4 bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col justify-between">
        <div>
          ...
          <p className="text-xs text-slate-450 mt-1 lines-clamp-2">
        ...
        <div className="pt-6 mt-6 border-t border-slate-100 space-y-4">
          ...
          <div className="flex justify-between text-[11px] font-medium">
            <span className="text-slate-400">Iniciado: 12 Sep 2023</span>
      ...
      <div className="lg:col-span-5 bg-slate-900 text-white rounded-2xl p-6 flex flex-col justify-between shadow-lg">
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-[9px] bg-slate-800 px-2 py-0.5 rounded text-slate-300 font-mono">Real vs Presupuestado</span>
          </div>
          
          <div className="space-y-3.5">
            {stats.hoursByStage.map(stg => {
              const stagePercent = Math.min(100, Math.round((stg.hours / (stg.budgetedHours || 1)) * 100)) || 0;
                ...
                <div key={stg.stageId} className="space-y-1">
                  <div className="flex justify-between text-xs font-semibold">
                    ...
                    <span className="text-slate-400 font-mono">
                      {stg.hours} / {stg.budgetedHours}h {stagePercent >= 100 && <span className="text-emerald-400">✓</span>}
                    </span>
                  </div>
        ...
        <div className="mt-6 pt-4 border-t border-slate-800/80">
          <div className="flex justify-between items-center">
            <span className="text-[10px] text-slate-450 uppercase font-bold tracking-wider">Costo Relacional Proyectado</span>
            <span className="text-lg font-black text-white font-mono">${stats.budgetedCost.toLocaleString("es-AR")}</span>
          </div>
        </div>
      </div>
      ...
      <div className="lg:col-span-3 flex flex-col gap-4">
        {/* Tu Actividad Semanal or average hour load stats */}
        <div className="bg-white rounded-2xl border border-slate-200 p-5 flex-1 flex flex-col justify-center">
          ...
          <div className="flex items-end gap-1.5 h-14 mb-3">
            ...
            <div className="bg-indigo-200 h-[40%] w-full rounded-t-lg"></div>
            <div className="bg-indigo-100 h-[20%] w-full rounded-t-lg"></div>
          </div>
          <p className="text-xl font-extrabold text-slate-850 font-sans tracking-tight">
            {stats.totalHours} <span className="text-xs font-normal text-slate-400 block mt-0.5">horas registradas</span>
          </p>
        </div>
